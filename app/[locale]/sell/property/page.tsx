...
import {
  Building2, MapPin, Loader2, CheckCircle, Trash2, ImageIcon, CreditCard,
  Copy, Check, Star, Zap, ChevronRight, ChevronLeft,
} from "lucide-react";
...
export default function SellPropertyPage() {
  ...
  const handleNext = () => {
    ...
    if (!formData.area) {
      setError(locale === "ar" ? "المساحة مطلوبة" : "La superficie est requise.");
      return;
    }
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center px-4">
        ...
        >
          ...
          <button type="button" onClick={() => router.push("/")}
            className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-2xl transition-all">
            {t("go_home")}
          </button>
        </motion.div>
      </div>
  ...
  const inputCls = "w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white rounded-2xl py-3 px-4 outline-none focus:ring-2 focus:ring-blue-500 placeholder:text-gray-400 dark:placeholder:text-gray-600 transition-colors";
        ...
        <div className="flex items-center gap-3 mb-8">
          <div className="flex items-center gap-2 shrink-0">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-black transition-all ${
              step >= 1 ? "bg-blue-600 text-white" : "bg-gray-200 dark:bg-white/10 text-gray-400 dark:text-gray-500"
            }`}>
              {step > 1 ? "✓" : "1"}
            </div>
          ...
          <div className={`flex-1 h-0.5 rounded-full transition-colors ${step > 1 ? "bg-blue-600" : "bg-gray-200 dark:bg-white/10"}`} />
          <div className="flex items-center gap-2 shrink-0">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-black transition-all ${
              step === 2 ? "bg-blue-600 text-white" : "bg-gray-200 dark:bg-white/10 text-gray-400 dark:text-gray-500"
            }`}>
              2
            </div>
          ...
          {step === 1 && (
            <>
              ...
              <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-white/10 rounded-3xl p-6 space-y-5 shadow-sm dark:shadow-none">
                ...
                {/* Property Type */}
                <div>
                  <label className={labelCls}>{tAdmin("type")}</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className={labelCls}>{tAdmin("area")}</label>
                    <input type="number" placeholder="0" className={inputCls}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className={labelCls}>{tAdmin("bedrooms")}</label>
                    <input type="number" placeholder="0" className={inputCls}
                      value={formData.bedrooms} onChange={(e) => setFormData({ ...formData, bedrooms: e.target.value })}
                    />
                  </div>
                  <div>
                    <label className={labelCls}>{tAdmin("bathrooms")}</label>
                    <input type="number" placeholder="0" className={inputCls}
                    ...
                    <CldUploadWidget uploadPreset={uploadPreset} onSuccess={handleUploadSuccess} options={{ multiple: true, maxFiles: 10 }}>
                      {({ open }) => (
                        <button type="button" onClick={() => open()}
                          className="w-full h-36 border-2 border-dashed border-gray-200 dark:border-white/10 hover:border-blue-500/40 rounded-2xl flex flex-col items-center justify-center gap-3 transition-all group">
                          <ImageIcon className="w-8 h-8 text-gray-400 dark:text-gray-600 group-hover:text-blue-400" />
                          <span className="text-sm text-gray-400 dark:text-gray-500 group-hover:text-blue-400">{tAdmin("add_photos")}</span>
                        </button>
                      )}
                    </CldUploadWidget>
                  ) : null}
              ...
              <button type="button" onClick={handleNext}
                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-5 rounded-3xl shadow-xl shadow-blue-500/20 flex items-center justify-center gap-3 transition-all active:scale-[0.98] text-lg">
          ...
          {step === 2 && (
            <>
              ...
              <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-white/10 rounded-2xl p-4 flex items-center gap-3 shadow-sm dark:shadow-none">
                <div className="w-10 h-10 bg-blue-50 dark:bg-blue-600/20 rounded-xl flex items-center justify-center shrink-0">
                  <Building2 className="w-5 h-5 text-blue-600 dark:text-blue-400" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-xs text-gray-400 dark:text-gray-500">{locale === "ar" ? "إعلانك" : "Votre annonce"}</div>
                  <div className="font-bold text-gray-900 dark:text-white truncate">
                    {tCat(formData.type === "HOUSE" ? "houses" : formData.type === "APARTMENT" ? "apartments" : formData.type === "LAND" ? "lands" : "buildings")}
              ...
              <div className="flex gap-3">
                ...
                <button type="submit" disabled={loading}
                  className="flex-[2] bg-blue-600 hover:bg-blue-700 text-white font-bold py-5 rounded-3xl shadow-xl shadow-blue-500/20 flex items-center justify-center gap-3 transition-all active:scale-[0.98] disabled:opacity-70 text-base">
